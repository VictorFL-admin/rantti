...
export default function PanelVendedoresContent({ refreshTrigger }: PanelVendedoresProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Lista Completa de Publicaciones de Venta */}
      {listings.length > 0 && (
        <div className="space-y-4">
          <h2 className="font-['Poppins',sans-serif] text-lg font-semibold text-gray-900">
            Todas tus publicaciones de venta
          </h2>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-1 mb-4">
                      ...
                      <p className="font-['Poppins',sans-serif] text-xs text-gray-500">
                        ...
                        {listing.unread_messages_count ? ` • ${listing.unread_messages_count} mensajes sin leer` : ''}
                      </p>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {listing.status === 'publicado' && (
                        <Button 
                          onClick={async () => {
                            try {
                              await UserListingsAPI.markAsSold(listing.id);
                              toast.success('Publicación marcada como vendida');
                              loadSalesData();
                            } catch (error) {
                              toast.error('Error al marcar como vendida');
                      ...
                      {listing.status === 'publicado' && (
                        <Button 
                          onClick={async () => {
                            try {
                              await UserListingsAPI.repostListing(listing.id);
                              toast.success('Publicación impulsada exitosamente');
                              loadSalesData();
                            } catch (error) {
                              toast.error('Error al impulsar publicación');
      ...
      {!loading && listings.length === 0 && (
        <Card className="bg-white border border-gray-200 rounded-2xl p-12 text-center">
          <p className="text-gray-500 mb-3">Aún no tienes publicaciones de venta</p>
